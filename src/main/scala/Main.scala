...
abstract class Visitor(cb: CheckBoard) {
  ...
  def nextMove(): Option[Cell]
}

class RandomVisitor(cb: CheckBoard) extends Visitor(cb) {
  def nextMove(): Option[Cell] =
    Random
      .shuffle(cb.possibleMoves(steps.last))
      .lastOption
}

class WarnsdorfVisitor(cb: CheckBoard) extends RandomVisitor(cb) {
  override def nextMove(): Option[Cell] = {
    val cells = cb.possibleMoves(steps.last).filterNot(isVisited)
    if (cells.isEmpty) {
      super.nextMove()
    } else {
...
class RailwayVisitor10(cb: CheckBoard) extends Visitor(cb) {
  ...
  private var iterator: Iterator[Cell] = Iterator.empty

  override def startWalking(cell: (Size, Size)) = {
    super.startWalking(cell)
  ...
  override def nextMove(): Option[Cell] = {
    ...
    require(cb.possibleMoves(steps.last).contains(next))
    Option(next)
  }
}
